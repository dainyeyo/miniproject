...
## 📌 31단계: 대기실 플레이어 목록 스크롤 제거 및 AI 드로잉 모드 텍스트 수정
### 사용자의 지시 프롬프트 원문
> 사진처럼 밑에 스크롤이 생겼는데 안생기게 크기 조절해줘 그리고 AI 드로잉 모드 (SD-Turbo 연동) 이거를 그냥 AI 드로잉 모드 라고 수정해줘

### 기술적 해결책 및 아키텍처 의사결정(ADR) 요약
1. **슬롯 목록 뷰포트 임계치 상향**: `player-slots-list`의 `max-height`를 기존 `480px`에서 `520px`로 늘림으로써 6인 기준의 플레이어 슬롯 픽셀 높이 총합을 충분히 수용하여 세로 스크롤바 발생 원인을 제거했습니다.
2. **레이아웃 왜곡 방지 및 가로 스크롤 가드**: 수축 현상이나 미세 픽셀 초과로 인한 부차적인 가로 스크롤 생성을 억제하기 위해 `overflow-x: hidden;`을 결합했습니다.
3. **사용자 경험(UX) 개선을 위한 텍스트 단순화**: 모드 선택 화면에서 불필요하게 명시된 정적 연동 부호인 `(SD-Turbo 연동)` 접미사를 제거하여 시각적 직관성을 향상했습니다.

---
### 🕒 2026-07-02 14:46 - 대기실 플레이어 목록 스크롤 제거 및 AI 드로잉 모드 텍스트 수정 완료
- **변경 목적**: 대기실 플레이어 목록 영역 내 불필요한 스크롤바 노출 억제 및 직관성을 위한 모드 설명 텍스트 단순화
- **수정/추가된 파일**:
  - [globals.css](file:///c:/MiniProject/miniproject/app/globals.css) (수정)
  - [page.js](file:///c:/MiniProject/miniproject/app/page.js) (수정)
- **세부 변경점**:
  - `app/globals.css` 내 `.player-slots-list` 클래스에 `max-height: 520px;` 및 `overflow-x: hidden;`을 반영하여 레이아웃 내 스크롤 현상 정비.
  - `app/page.js` 내 AI 드로잉 모드 설명 카드 제목에서 `(SD-Turbo 연동)` 서픽스를 삭제하여 텍스트 간소화.

## 📌 32단계: 대기실 플레이어 슬롯 1열 레이아웃 확장 및 강퇴 버튼(X) 잘림 핫픽스
### 사용자의 지시 프롬프트 원문
> 이러니까 X가 잘리는데 X 버튼이 제대로 나오는데 밑에 스크롤이 안생기게 전체 네모박스 크기를 조절하든 해서 다 제대로 나오게 해

### 기술적 해결책 및 아키텍처 의사결정(ADR) 요약
1. **슬롯 목록 1열 가로폭 확장**: `.waiting-room-body`의 플레이어 슬롯 컬럼 가로폭 사양을 `240px`에서 `260px`로 확장하여 X 버튼이 튀어나갈 수 있는 물리적인 가용 공간을 추가했습니다.
2. **클리핑 해제 및 가로/세로 스크롤 완전 차단**: `.player-slots-list`에 걸렸던 세로 `max-height` 및 가로 `overflow-x: hidden` 속성을 걷어내고 `overflow: visible`을 지정했습니다. 이를 통해 스크롤바의 생성을 완벽히 억제하는 동시에 부모 경계를 넘나드는 절대 좌표 강퇴 버튼(X)이 잘리지 않도록 조치했습니다.
3. **상단 및 우측 완충 영역 설정**: `.player-slots-list` 컨테이너의 패딩을 `padding: 10px 10px 10px 0;`으로 설계하여 상단과 우측으로 삐져나온 X 버튼이 다른 컨테이너의 오버플로우 클리핑 속성에 영향받지 않고 온전한 구형으로 노출되도록 보정했습니다.

---
### 🕒 2026-07-02 14:51 - 대기실 플레이어 슬롯 1열 레이아웃 확장 및 강퇴 버튼(X) 잘림 핫픽스 완료
- **변경 목적**: 강퇴 버튼(X 마크)이 대기실 플레이어 카드 꼭짓점 밖으로 오프셋될 때 overflow 속성 및 컨테이너 벽에 닿아 잘려 보이는 현상 해결
- **수정/추가된 파일**:
  - [globals.css](file:///c:/MiniProject/miniproject/app/globals.css) (수정)
- **세부 변경점**:
  - `app/globals.css` 내 `.waiting-room-body` 1열 폭을 `260px`로 확대하고, `.player-slots-list` 의 스크롤 속성을 제거 및 `overflow: visible` 처리하고 `padding: 10px 10px 10px 0;` 버퍼 패딩을 적용함.
